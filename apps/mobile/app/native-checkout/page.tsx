...
import {nativePaymentsAvailable,startNativePayment} from "../payments/native-payment-bridge";
...
function labelFor(id:string){const x=id.toLowerCase();if(x.includes("amwal"))return"أموال";if(x.includes("tabby"))return"تابي";if(x.includes("tamara"))return"تمارا";if(x.includes("apple"))return"Apple Pay";if(x.includes("mada"))return"مدى";if(x.includes("stc"))return"STC Pay";if(x.includes("tap"))return"Tap";if(x.includes("card")||x.includes("credit"))return"البطاقات";return id.replace(/[_-]+/g," ")}
function sleep(ms:number){return new Promise(r=>setTimeout(r,ms))}
function normalizedStatus(value:string){const s=(value||"").toLowerCase();if(["processing","completed"].includes(s))return"paid";if(s==="failed")return"failed";if(["cancelled","refunded"].includes(s))return"cancelled";return"pending"}

export default function NativeCheckout(){
 const router=useRouter();
 const [cart,setCart]=useState<Cart|null>(null),[selected,setSelected]=useState(""),[busy,setBusy]=useState(false),[error,setError]=useState(""),[result,setResult]=useState("");
 useEffect(()=>{(async()=>{try{const cp=await readJson(await fetch("/api/store/cart",{cache:"no-store"}));setCart(cp);setSelected(cp.payment_methods?.[0]||"")}catch{setError("تعذر تجهيز وسائل الدفع")}})()},[]);
 const total=useMemo(()=>cart?amount(cart.totals.total_price,cart.totals.currency_minor_unit):0,[cart]);

 async function verifyOrder(orderId:string,orderKey:string,email:string){
   let last="pending";
   for(let i=0;i<10;i++){
     const q=new URLSearchParams({key:orderKey});if(email)q.set("billing_email",email);
     const r=await fetch(`/api/store/order/${encodeURIComponent(orderId)}?${q.toString()}`,{cache:"no-store"});
     if(r.ok){const order=await r.json();last=normalizedStatus(String(order?.status||""));if(last!=="pending")return last}
 ...
 async function pay(){
   ...
   try{
     ...
     const shipping={...(cart.shipping_address||{}),email:"",phone:""};
     ...
     const orderId=String(checkout?.order_id||checkout?.id||"");
     const orderKey=String(checkout?.order_key||checkout?.key||"");
     ...
     const checkoutUrl=String(target?.url||"");
     if(!checkoutUrl.startsWith("https://"))throw new Error("تعذر تجهيز صفحة الدفع داخل التطبيق");
     const nativeResult=await startNativePayment({provider:selected,orderId,orderKey,checkoutUrl});
     if(nativeResult.status==="cancelled"){setResult(`تم إلغاء عملية الدفع للطلب رقم ${orderId}`);return}
     if(nativeResult.status==="failed")throw new Error(nativeResult.message||"تعذر فتح بوابة الدفع");
