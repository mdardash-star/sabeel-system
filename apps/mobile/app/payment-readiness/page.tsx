...
export default function PaymentReadiness(){
 ...
 return <main style={s.page} dir="rtl"><section style={s.card}>
  ...
  {data.methods.length?<div style={s.grid}>{data.methods.map(m=><article key={m.id} style={s.method}>
   <div style={s.methodHead}><b style={s.methodName}>{m.label}</b><span style={m.status==="ready"?s.ready:s.needs}>{m.status==="ready"?"جاهزة":"تحتاج إعداد"}</span></div>
   <p style={m.status==="ready"?s.okText:s.warnText}>{m.status==="ready"?"WooCommerce يدير الجلسة والبوابة، والتطبيق يفتحها داخل نافذة الدفع الأصلية.":`ينقص الإعداد: ${m.missing.join("، ")||"إعداد البوابة"}`}</p>
