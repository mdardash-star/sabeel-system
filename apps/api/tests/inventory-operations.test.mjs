...
function transactionalClient(handler){const calls=[];return{calls,db:{query:async()=>({rows:[]}),connect:async()=>({query:async(sql,params=[])=>{calls.push({sql,params});if(['BEGIN','COMMIT','ROLLBACK'].includes(sql))return{rows:[]};if(/SELECT organization_id FROM users/.test(sql))return{rows:[{organization_id:ORG}]};return handler(sql,params);},release(){}})}};}

test('warehouse receipt increases balance and records movement and audit',async()=>{
 const {db,calls}=transactionalClient(async(sql,params)=>{if(/CROSS JOIN warehouses/.test(sql))return{rows:[{id:'i1',warehouse_id:'main'}]};if(/INSERT INTO inventory_balances/.test(sql))return{rows:[{organization_id:ORG,warehouse_id:'main',item_id:'i1',quantity:'10'}]};if(/UPDATE inventory_items/.test(sql))return{rows:[]};if(/INSERT INTO inventory_movements/.test(sql))return{rows:[{id:'mv1',movement_type:'receipt'}]};if(/INSERT INTO audit_log/.test(sql))return{rows:[]};throw new Error(`unexpected ${sql}`);});
 const result=await receiveInventory(db,{warehouseId:'main',itemId:'i1',quantity:10,unitCost:40,reference:'PO-1',notes:'',actorUserId:'m1'});assert.equal(result.balance.quantity,'10');assert.ok(calls.some(call=>call.sql==='COMMIT'));assert.ok(calls.some(call=>/inventory.received/.test(call.params[1]||'')));assert.ok(calls.some(call=>/organization_id/.test(call.sql)));
});

function transferHandler(balance='10') {return async(sql,params)=>{if(/SELECT id FROM warehouses/.test(sql))return{rows:[{id:'secondary'}]};if(/SELECT warehouse_id, item_id, quantity/.test(sql))return{rows:[{warehouse_id:'main',item_id:'i1',quantity:balance}]};if(/UPDATE inventory_balances/.test(sql))return{rows:[]};if(/INSERT INTO inventory_balances/.test(sql))return{rows:[{organization_id:ORG,warehouse_id:'secondary',item_id:'i1',quantity:params[3]}]};if(/INSERT INTO inventory_movements/.test(sql))return{rows:[{id:'mv2',movement_type:'transfer'}]};if(/INSERT INTO audit_log/.test(sql))return{rows:[]};throw new Error(`unexpected ${sql}`);};}
...
test('technician issue deducts warehouse and credits custody atomically',async()=>{
 const {db,calls}=transactionalClient(async(sql,params)=>{if(/FROM technicians t JOIN users u/.test(sql))return{rows:[{id:'t1'}]};if(/SELECT warehouse_id, item_id, quantity/.test(sql))return{rows:[{quantity:'8'}]};if(/UPDATE inventory_balances/.test(sql))return{rows:[]};if(/INSERT INTO technician_inventory/.test(sql))return{rows:[{organization_id:ORG,technician_id:'t1',item_id:'i1',quantity:2}]};if(/INSERT INTO inventory_movements/.test(sql))return{rows:[{id:'mv3',movement_type:'technician_issue'}]};if(/INSERT INTO audit_log/.test(sql))return{rows:[]};throw new Error(`unexpected ${sql}`);});
