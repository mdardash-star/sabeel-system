...
test('live HTTP inventory worklist forwards stock filter and pagination',async(t)=>{
  const db={query:async(sql,params)=>{assert.match(sql,/stock_status/);assert.deepEqual(params,[organizationId,'ممبرين','low',5,10]);return{rows:[{id:'i1',sku:'MEM-75',stock_status:'low',total_count:3}]};}};const server=createApiServer({db:withSession(db,{role:'branch_manager'})});await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));t.after(()=>new Promise(resolve=>server.close(resolve)));
...
test('live HTTP manager receives warehouse stock atomically',async(t)=>{
  const client={query:async(sql,params=[])=>{if(['BEGIN','COMMIT'].includes(sql))return{rows:[]};if(/SELECT organization_id FROM users/.test(sql))return{rows:[{organization_id:organizationId}]};if(/CROSS JOIN warehouses/.test(sql))return{rows:[{id:'i1',warehouse_id:'main-riyadh'}]};if(/INSERT INTO inventory_balances/.test(sql))return{rows:[{warehouse_id:'main-riyadh',item_id:'i1',quantity:'5'}]};if(/UPDATE inventory_items/.test(sql))return{rows:[]};if(/INSERT INTO inventory_movements/.test(sql))return{rows:[{id:'mv1',movement_type:'receipt'}]};if(/INSERT INTO audit_log/.test(sql))return{rows:[]};throw new Error(`unexpected query: ${sql}`);},release(){}};
