...
test('dispatcher reads ranked candidates for a dispatchable job',async()=>{
  ...
  const result=await routePersistentRequest({method:'GET',url:'/api/v1/jobs/job-1/candidates',role:'dispatcher',context:{limit:'5',tenantId:'org-1'},db});
  assert.equal(result.status,200);assert.equal(result.data.candidates[0].technician_id,'tech-1');assert.equal(calls[0].params[1],'org-1');assert.equal(calls[1].params[8],'org-1');
});

function assignmentDb(status='pending_assignment'){
  const calls=[];const client={query:async(sql,params=[])=>{calls.push({sql,params});if(['BEGIN','COMMIT','ROLLBACK'].includes(sql))return{rows:[]};if(/service_jobs WHERE id=\$1 AND/.test(sql))return{rows:[{id:'job-1',organization_id:'org-1',city_id:'riyadh',status,service_duration_minutes:60,technician_id:'old-tech',scheduled_at:'2026-09-20T09:00:00Z'}]};if(/FROM technicians t JOIN users/.test(sql))return{rows:[{id:'tech-1',city_id:'riyadh'}]};if(/FROM technician_availability/.test(sql))return{rows:[{'?column?':1}]};if(/SELECT id FROM service_jobs/.test(sql))return{rows:[]};if(/UPDATE service_jobs/.test(sql))return{rows:[{id:'job-1',technician_id:'tech-1',scheduled_at:params[2],service_duration_minutes:params[3],status:'scheduled'}]};if(/INSERT INTO audit_log/.test(sql))return{rows:[]};throw new Error(`unexpected query: ${sql}`);},release(){}};return{db:{query:async()=>({rows:[]}),connect:async()=>client},calls};
}

test('dispatcher assigns and schedules through persistent HTTP router',async()=>{
  const {db,calls}=assignmentDb();const result=await routePersistentRequest({method:'POST',url:'/api/v1/jobs/job-1/assign',role:'dispatcher',context:{userId:'dispatcher-1',tenantId:'org-1'},body:{technicianId:'tech-1',scheduledAt:'2026-09-20T10:00:00Z',serviceDurationMinutes:90},db});
