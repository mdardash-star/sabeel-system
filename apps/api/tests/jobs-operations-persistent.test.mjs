...
test('dispatcher reads operations job counters',async()=>{
  const db={query:async(sql)=>{assert.match(sql,/pending_assignment/);assert.match(sql,/completed_today/);return{rows:[{open:12,pending_assignment:3,scheduled_today:4,in_progress:2,overdue:1,completed_today:8}]};}};
  const result=await routePersistentRequest({method:'GET',url:'/api/v1/jobs/stats',role:'dispatcher',db});
  assert.equal(result.status,200);assert.equal(result.data.stats.overdue,1);
});

test('support searches and filters paginated operations jobs',async()=>{
  const db={query:async(sql,params)=>{assert.match(sql,/JOIN customers/);assert.match(sql,/sla_state/);assert.match(sql,/WHEN 'overdue'/);assert.deepEqual(params,['محمد','overdue',10,20]);return{rows:[{id:'job-1',customer_name:'محمد',sla_state:'overdue',minutes_late:45,total_count:6}]};}};
  const result=await routePersistentRequest({method:'GET',url:'/api/v1/jobs',role:'support',context:{query:'محمد',status:'overdue',limit:'10',offset:'20'},db});
