...
export async function createSegment(db, input) {
  return withTransaction(db, async client => {
    const segment = (await client.query(
      `INSERT INTO customer_segments (name, segment_type, created_by) VALUES ($1,$2,$3) RETURNING *`,
      [input.name, input.segmentType, input.actorUserId]
    )).rows[0];
    await audit(client, input.actorUserId, 'marketing.segment_created', 'customer_segment', segment.id, { segmentType: input.segmentType });
    return segment;
  });
}
...
export async function createCampaign(db, input) {
  return withTransaction(db, async client => {
    const segment = (await client.query('SELECT id FROM customer_segments WHERE id=$1 AND is_active=true', [input.segmentId])).rows[0];
    if (!segment) throw new Error('Marketing segment not found');
    const campaign = (await client.query(
      `INSERT INTO marketing_campaigns (name,segment_id,channel,message,status,scheduled_at,created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [input.name,input.segmentId,input.channel,input.message,input.scheduledAt?'scheduled':'draft',input.scheduledAt||null,input.actorUserId]
    )).rows[0];
    await audit(client,input.actorUserId,'marketing.campaign_created','marketing_campaign',campaign.id,{channel:input.channel,scheduledAt:input.scheduledAt||null});
    return campaign;
  });
}
...
export async function launchCampaign(db, { campaignId, actorUserId }) {
  return withTransaction(db, async client => {
    const campaign = (await client.query(
      `SELECT mc.*,cs.segment_type FROM marketing_campaigns mc JOIN customer_segments cs ON cs.id=mc.segment_id
       WHERE mc.id=$1 FOR UPDATE OF mc`, [campaignId]
    )).rows[0];
    if (!campaign) return null;
    if (!['draft','scheduled'].includes(campaign.status)) throw new Error('Campaign is not launchable');
    const audience = await selectAudience(client, campaign.segment_type);
    for (const customer of audience) {
      ...
      await client.query(
        `INSERT INTO campaign_recipients (campaign_id,customer_id,notification_event_id)
         VALUES ($1,$2,$3) ON CONFLICT (campaign_id,customer_id) DO NOTHING`, [campaign.id,customer.id,event.id]
      );
    }
    const updated = (await client.query(
      `UPDATE marketing_campaigns SET status='queued',launched_at=now(),updated_at=now() WHERE id=$1 RETURNING *`, [campaign.id]
    )).rows[0];
    await audit(client,actorUserId,'marketing.campaign_launched','marketing_campaign',campaign.id,{audienceCount:audience.length,channel:campaign.channel});
    return { campaign:updated, audienceCount:audience.length };
  });
}

export async function previewAudience(db, segmentType, limit = 10) {
  const rows = await selectAudience(db, segmentType, limit);
  return { count: Number(rows[0]?.audience_count || rows.length), customers: rows.map(({ audience_count, ...customer }) => customer) };
}

async function selectAudience(client, segmentType, limit = null) {
  const conditions = {
    all: 'true',
  ...
  const sql = `SELECT c.id,c.name,u.mobile,COUNT(*) OVER()::integer AS audience_count
    FROM customers c LEFT JOIN users u ON u.id=c.user_id WHERE u.is_active IS DISTINCT FROM false AND ${condition}
    ORDER BY c.created_at DESC${limit ? ' LIMIT $1' : ''}`;
  return (await client.query(sql, limit ? [limit] : [])).rows;
}
