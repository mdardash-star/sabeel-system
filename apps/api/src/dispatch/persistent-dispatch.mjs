...
export async function listDispatchCandidates(db, jobId, { windowStart, windowEnd, limit = 10 } = {}) {
  const jobResult = await db.query(`SELECT j.id, j.city_id, j.scheduled_at, j.status, j.required_skill_code, j.service_duration_minutes, l.latitude, l.longitude FROM service_jobs j LEFT JOIN service_locations l ON l.id=j.service_location_id WHERE j.id=$1 LIMIT 1`, [jobId]);
...
async function validateAssignment(client, job, technicianId, scheduledAt, durationMinutes) {
...
export async function assignPersistentJob(db,{jobId,technicianId,scheduledAt,serviceDurationMinutes,windowMinutes,actorUserId}){
  return withTransaction(db,async client=>{
    ...
    const {startIso}=await validateAssignment(client,job,technicianId,scheduledAt,durationMinutes);
...
export async function reassignPersistentJob(db,{jobId,technicianId,scheduledAt,serviceDurationMinutes,windowMinutes,actorUserId,reason}){
