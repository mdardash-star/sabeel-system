    ...
    `SELECT t.id AS technician_id, t.city_id, t.branch_id,
            ...
            CASE WHEN $7::numeric IS NULL OR $8::numeric IS NULL OR tl.latitude IS NULL OR tl.longitude IS NULL THEN NULL
              ELSE ROUND((6371 * 2 * ASIN(SQRT(
                POWER(SIN(RADIANS((tl.latitude - $7::numeric) / 2)), 2) +
                COS(RADIANS($7::numeric)) * COS(RADIANS(tl.latitude)) *
                POWER(SIN(RADIANS((tl.longitude - $8::numeric) / 2)), 2)
              )))::numeric, 2) END AS distance_km
     FROM technicians t
