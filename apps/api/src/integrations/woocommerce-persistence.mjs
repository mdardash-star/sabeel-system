...
export async function persistPaidServiceOrder(db, order, { cityId = 'riyadh', organizationId = DEFAULT_ORG, client = null } = {}) {
  ...
  const persist = async (transaction) => {
    ...
    if (existingOrder) {
      ...
      if(attribution.meaningful){
        const hasAttribution=(await transaction.query('SELECT 1 FROM order_attribution WHERE order_id=$1 LIMIT 1',[existingOrder.id])).rows[0];
        if(!hasAttribution){
          const touch=(await transaction.query(
      ...
      const existingJob=(await transaction.query('SELECT * FROM service_jobs WHERE order_id=$1 LIMIT 1',[existingOrder.id])).rows[0]||null;
    ...
    const persistedOrder = (await transaction.query(
    ...
    const costRow = (await transaction.query(
    ...
    let job=null;
    if(mapped){
      const serviceLocation = (await transaction.query(
  ...
  return client ? persist(client) : withTransaction(db, persist);
}
